...
func Publish(file *multipart.FileHeader, title string, user_id uint) error {
	...
	playUrl, coverUrl, err := upload(file.Filename, user.Username, video.Id)
	if err != nil {
...
func upload(fileName string, username string, video_id uint) (string, string, error) {
	bucketName := "simple-tiktok-" + username + "-bucket"
	objectName := "video/" + strconv.FormatUint(uint64(video_id), 10)
	
	playUrl, err := ossService.Upload(fileName, bucketName, objectName)
	if err != nil {
		return "", "", err
	}
